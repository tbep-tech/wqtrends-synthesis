...
Changing the knots for `cont_year` and `ssc` can be done with the `kts` and `ktssc` arguments.
...
The original GAM model without ssc can be created setting the `usessc` argument to `FALSE`. 
